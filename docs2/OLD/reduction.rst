...
KASTREDUX performs a standard suite of reduction procedures for optical spectroscopy, optimized for point source extraction and calibration. The following are the steps that KASTREDUX will undertake:
  ...
  * Tracing of curved spectral dispersions
  * Source extraction using boxcar and PSF spatial profiles with background subtraction
...
Necessary Commands (for data reduction)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
OUTPUT_FOLDER
  full path to where the final spectral data products should go; i.e., "DATE/final" in the architecture given above; if not provided, the output goes into the REDUCTION_FOLDER
...
It is assumed these files reside in the ``REDUCTION_FOLDER``. If they are not found, these reductions are recomputed.
