...
# single number = turn into small range
	...
	def applyMask(self,mask=[],replace=True,replace_value='local',local_range=50):
		...
		elif isNumber(mask[0]):
			wok = numpy.where(mask==0)
			wrej = numpy.where(mask==1)
		else:
			print('mask array needs to be 1/0 or True/False, no action taken')
			return
		if len(wok[0])>0:
			if replace==False:
...
# check for relevant parameters
	...
	def redden(self, av=0.0, rv=3.1, normalize=False, a=10., n=1.33, **kwargs):
		...
		**Note**
		...
		'''
		w = self.wave.to(DEFAULT_WAVE_UNIT).value   # assuming in Angstroms
	...
	#		tau0 = numpy.pi*(a**2)*(qabs0+qsca0)
		...
		self.unc = numpy.array(self.unc.value)*numpy.array(absfrac)*self.unc.unit
		self.variance = self.unc**2
...
#		fig.show()
	...
	def toFile(self,file,overwrite=True,csv=False,delimiter='\t',save_header=True,save_noise=False,save_background=False,save_mask=False,comment='#',**kwargs):
		...
		NEED TO REDO THIS TO SAVE TO CSV AND OTHER TYPES OF FILES USING PANDAS
		'''
# what are we saving?
...
# occasional issue - images read in are wrong size - warn user
	for im in images:
		if numpy.shape(im) != numpy.shape(images[0]):
			raise ValueError('Warning: all frames are not the same size - check your files for duplicates')
...
# prior coefficient provided - use this
	if 'COEFF' in list(prior.keys()):
		p1 = prior['COEFF']
		wave1 = numpy.polyval(p1,numpy.arange(len(arctrace)))
	else:
# if initial values of pixel wavelength provided, construct initial guess from given values
		if len(pixel0) > 0 and len(lam0) > 0:
			p0 = numpy.polyfit(pixel0,lam0,int(numpy.nanmin([sfit_order,len(lam0)-1])))
# guess for brightest pixel but not wavelength: adopt pixel as assumed strongest line, and assume linear model based on resolution
		elif len(pixel0) > 0 and len(lam0) == 0:
			p0 = [resolution,strongest-resolution*numpy.argmax(pixel0[0])]
# guess for brightest wavelength but not pixel: adopt strongest line as wavelength given, and assume linear model based on resolution
		elif len(pixel0) == 0 and len(lam0) > 0:
			p0 = [resolution,lam0[0]-resolution*numpy.argmax(arctrace)]
# no guess for pixel of wavelength: center on strongest line, and adopt linear model based on resolution
		else:
			p0 = [resolution,strongest-resolution*numpy.argmax(arctrace)]
		wave0 = numpy.polyval(p0,numpy.arange(len(arctrace)))
	# pixel0 = [int(x) for x in pixel0]
	# else: 
	# 	if len(lam0) == 0: raise ValueError('Problem: there is no initial estimate of central wavelength to perform wavelength calibration')
...
def initializeStandards(spt=[],sdss=True,folder=SPTSTDFOLDER,reset=False,sd=False,esd=False,usd=False,beta=False,giant=False,gamma=False,z=0.,ref='',verbose=ERROR_CHECKING):
	"""
	...
	if isinstance(spt[0],float) == True or isinstance(spt[0],int) == True: 
		if len(spt) == 2: spt = numpy.arange(spt[0],spt[1]+1,0.5)
	...
	for s in spt:
		if s not in list(SPTSTDS.keys()) or reset==True:
			...
			if len(f) > 1:
				...
				if giant != True: f = f[['I_' not in x for x in f]]
				if z != 0 and len(f[['-Z' in x for x in f]])>0: 
					if z > 0: f = f[['-Z+{:.1f}'.format(z) in x for x in f]]
					else: f = f[['-Z-{:.1f}'.format(z) in x for x in f]]
				else: f = f[['-Z' not in x for x in f]]
			if len(f) > 0:
